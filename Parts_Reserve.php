<?php
	...
	$ResultCount = GetPartsForReserveCount($MSN, $KIT, $DWG, $VEN, $PN, $SN, $LOC, $BIN, $WP, $PROD);

	$PageCounter = GeneratePageCounter($CurrentPage, $ResultCount, 100, "Parts_Reserve.php?MSN=" . $MSN . "&KIT=" . $KIT . "&DWG=" . $DWG . "&VEN=" . $VEN . "&PN=" . $PN . "&SN=" . $SN . "&LOC=" . $LOC . "&BIN=" . $BIN . "&WP=" . $WP . "&PROD=" . $PROD);
...
<body onLoad="init()">
	...
	<SCRIPT LANGUAGE="JavaScript">
		...
		function checkAll() {
			for (i = 0; i < document.Reserve.elements.length; i++) {
				if(document.Reserve.elements[i].type=="checkbox") {
					document.Reserve.elements[i].checked=true;
				}
			}
		}
		...
		function uncheckAll() {
			for (i = 0; i < document.Reserve.elements.length; i++) {
				if(document.Reserve.elements[i].type=="checkbox") {
					document.Reserve.elements[i].checked=false;
				}
			}
		}
		...
		function CheckValues() {
			...
			for (i = 0; i < document.Reserve.elements.length; i++) {
				if(document.Reserve.elements[i].type=="checkbox") {
					if(document.Reserve.elements[i].checked==true) {
						PNCOUNT++;
					}
				}
			}
		...
		function SendValues() {
			...
			for (i = 0; i < document.Reserve.elements.length; i++) {
				if(document.Reserve.elements[i].type=="checkbox") {
					if(document.Reserve.elements[i].checked==true) {
						PNID = PNID + document.Reserve.elements[i].value + "/";
					}
				}
			}

			URL = "Parts_Reserve_TaskCard.php?PNID=" + PNID;

			window.open(URL,'ReserveWindow','height=500,width=800,left=10,top=10,resizable=yes,scrollbars=yes,toolbar=no,menubar=no,location=no,directories=no,status=no');
		}
	...
	<form name="Reserve" method="post" action="Parts_Reserve_Multi_Action.php">
		...
		<a href="javascript:void(checkAll(document.Reserve.DATA));">Check All</a>
		<a href="javascript:void(uncheckAll(document.Reserve.DATA));">Uncheck All</a>
		<a href="javascript:void(CheckValues());" ><?php if($_SESSION["RIGHTS"] == 2) echo "Request Selected"; else echo "Reserve Selected"; ?></a>
		</center>
		...
		<table border="0" cellspace="1" width="100%">
			...
			<?php
				...
				GetPartsForReserve($MSN, $KIT, $DWG, $VEN, $PN, $SN, $LOC, $BIN, $WP, $PROD, $CurrentPage, 100, $result);
				...
				while ($ResultData = mysqli_fetch_assoc($result)) {
					...
					$DATA = $ResultData["PN_ID"] . "," . $ResultData["INVD_QTY"];

					$href = "javascript:void(window.open('Parts_Reserve_TaskCard.php?PNID=" . $DATA . "','ReserveWindow','height=500,width=800,left=10,top=10,resizable=yes,scrollbars=yes,toolbar=no,menubar=no,location=no,directories=no,status=no'));";
			?>
			...
			<tr>
				...
				<td valign="top" bgcolor="<?php echo $BGColor; ?>">
					<font size="2">
						<a href="<?php echo $href; ?>"><?php echo $ResultData["PN_PRODNUM"]; ?></a>
					</font>
				</td>
				<td valign="top" bgcolor="<?php echo $BGColor; ?>">
					<font size="2">
