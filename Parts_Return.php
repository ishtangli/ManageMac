<?php
	...
	$ResultCount = GetPartsForReturnCount($MSN, $KIT, $DWG, $VEN, $PN, $SN, $TRN, $WP);

	$PageCounter = GeneratePageCounter($CurrentPage, $ResultCount, 100, "Parts_Return.php?MSN=" . $MSN . "&KIT=" . $KIT . "&DWG=" . $DWG . "&VEN=" . $VEN . "&PN=" . $PN . "&SN=" . $SN . "&TRN=" . $TRN . "&WP=" . $WP);
...
<body onLoad="init()">
	...
	<SCRIPT LANGUAGE="JavaScript">
		...
		function checkAll() {
			for (i = 0; i < document.Return.elements.length; i++) {
				if(document.Return.elements[i].type=="checkbox") {
					document.Return.elements[i].checked=true;
				}
			}
		}
		...
		function uncheckAll() {
			for (i = 0; i < document.Return.elements.length; i++) {
				if(document.Return.elements[i].type=="checkbox") {
					document.Return.elements[i].checked=false;
				}
			}
		}
	...
	<form name="Return" method="post" action="Parts_Return_Multi_Action.php">
		...
		<table border="0" cellspace="1" width="100%">
			...
			<?php
				...
				GetPartsForReturn($MSN, $KIT, $DWG, $VEN, $PN, $SN, $TRN, $WP, $CurrentPage, 100, $result);
				...
				while ($ResultData = mysqli_fetch_assoc($result)) {
					...
					$href = "javascript:void(window.open('Parts_Return_Multi.php?INVTID=" . $ResultData["INVT_ID"] . "','ReturnWindow','height=500,width=800,left=10,top=10,resizable=yes,scrollbars=yes,toolbar=no,menubar=no,location=no,directories=no,status=no'));";
			?>
			...
			<tr>
				...
				<td valign="top" bgcolor="<?php echo $BGColor; ?>" align="right">
					<font size="2">
						<a href="<?php echo $href; ?>"><?php echo $ResultData["INVH_RETURN_QTY"]; ?></a>
					</font>
				</td>
				<td valign="top" bgcolor="<?php echo $BGColor; ?>">
					<font size="2">
				...
				<td valign="top" bgcolor="<?php echo $BGColor; ?>">
					<font size="2">
						<a href="<?php echo $href; ?>"><?php echo $ResultData["INVH_FROM_LOC"]; ?></a>
					</font>
				</td>
				<td valign="top" bgcolor="<?php echo $BGColor; ?>">
					<font size="2">
