<?php
	...
	$ResultCount = GetPartsForPositionCount($MSN, $KIT, $DWG, $VEN, $PN, $SN, $TRN, $WP);

	$PageCounter = GeneratePageCounter($CurrentPage, $ResultCount, 100, "Parts_Position.php?MSN=" . $MSN . "&KIT=" . $KIT . "&DWG=" . $DWG . "&VEN=" . $VEN . "&PN=" . $PN . "&SN=" . $SN . "&TRN=" . $TRN . "&WP=" . $WP);
...
<body onLoad="init()">
	...
	<SCRIPT LANGUAGE="JavaScript">
		...
		function checkAll() {
			for (i = 0; i < document.Position.elements.length; i++) {
				if(document.Position.elements[i].type=="checkbox") {
					document.Position.elements[i].checked=true;
				}
			}
		}
		...
		function uncheckAll() {
			for (i = 0; i < document.Position.elements.length; i++) {
				if(document.Position.elements[i].type=="checkbox") {
					document.Position.elements[i].checked=false;
				}
			}
		}
	...
	<form name="Position" method="post" action="Parts_Position_Multi_Action.php">
		...
		<table border="0" cellspace="1" width="100%">
			...
			<?php
				...
				GetPartsForPosition($MSN, $KIT, $DWG, $VEN, $PN, $SN, $TRN, $WP, $CurrentPage, 100, $result);
				...
				while ($ResultData = mysqli_fetch_assoc($result)) {
					...
					$href = "javascript:void(window.open('Parts_Position_Multi.php?INVTID=" . $ResultData["INVT_ID"] . "','PositionWindow','height=500,width=800,left=10,top=10,resizable=yes,scrollbars=yes,toolbar=no,menubar=no,location=no,directories=no,status=no'));";
			?>
