<?php
	...
	$ResultCount = GetPartsHistoryByKitCount($MSN, $KIT, $DWG, $VEN, $PN, $SN);

	$PageCounter = GeneratePageCounter($CurrentPage, $ResultCount, 100, "Parts_History_KIT.php?MSN=" . $MSN . "&KIT=" . $KIT . "&DWG=" . $DWG . "&VEN=" . $VEN . "&PN=" . $PN . "&SN=" . $SN);
...
<body onLoad="init()">
	...
	<table border="0" cellspace="1" width="100%">
		...
		<?php
			...
			GetPartsHistoryByKit($MSN, $KIT, $DWG, $VEN, $PN, $SN, $CurrentPage, 100, $result);
			...
			while ($ResultData = mysqli_fetch_assoc($result)) {
				...
				$href = "javascript:void(window.open('Parts_History_KIT_Details.php?PN=" . $ResultData["PN_ID"] . "','HistoryWindow','height=500,width=800,left=10,top=10,resizable=yes,scrollbars=yes,toolbar=no,menubar=no,location=no,directories=no,status=no'));";

				if (!is_null($ResultData["HISTORYPN"])) {
